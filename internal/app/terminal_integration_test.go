...
func TestTerminalIntegrationTemporaryFilesAndRestrictedFallback(t *testing.T) {
	...
	defer cancel()
	// A read-only home does not prevent /tmp staging. No writes target the home.
	s.Home = "/proc"
	...
	file, err := s.files.Open(path.Join(integration.directory, "bashrc"))
	if err != nil {
		t.Fatal(err)
	}
	file.Close()
	...
	if _, err = s.files.Stat(integration.directory); err == nil {
