...
(() => {
  'use strict';
  const sections = [
    { id: 'finalshell-import', title: '从 FinalShell 导入', icon: 'folder', html: `<p>在 FinalShell 中导出当前或全部 SSH 连接，将导出文件放进 <strong>finalshell_oot_pot</strong> 文件夹，再将此文件夹放在 DengShell 可执行程序旁。设置中点击“从 FinalShell 导入”，自动扫描并导入；密码自动解析并加密保存，无需 Python 插件。密钥导出只含引用时，点击“补充私钥”关联本机密钥。重复导入跳过已有记录，不覆盖手动修改的凭据。</p>` },
