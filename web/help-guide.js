...
(() => {
  ...
  const steps = [
    ...
    { section: 'quick-commands', title: '把常用操作保存为快捷命令', target: '[data-pane="commands"]', location: '下方面板 → 命令', text: '命令区域右键可新建分组，空组也会保留。为命令设置名称、分组、颜色和内容，每条命令显示为彩色胶囊。', bullets: ['新建或编辑命令时可选择已有分组，分组旁“＋”可创建新组。点击胶囊会在当前 SSH 会话中执行命令。', '发送前确认顶部会话，避免选错服务器。'] },
