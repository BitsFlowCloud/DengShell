...
下文四段动态图均来自实际界面操作，使用隔离的 SSH / SFTP 演示环境录制，帧率为 **15 FPS**。
...
| Windows 单独程序 | `DengShell.exe` | 放入可写目录运行；手动升级时保留原有 `data/` |
...
点击快捷命令会在当前 SSH 会话中执行，适合重复使用的查看日志、检查服务或查询资源等操作。
...
- 提示符的用户名与主机名可以分别配色；在兼容的 Bash / Zsh 中于下一次提示符显示时生效。
...
| Linux DEB、RPM 或通用安装版 | `${XDG_CONFIG_HOME:-$HOME/.config}/dengshell` |
...
启动时可以检查新版本，确认后下载并校验更新。Windows 和 Debian / Ubuntu 提供相应在线更新流程，其他 Linux 发行版通过对应安装包升级。
