...
(() => {
  ...
  const sections = [
    ...
    { id: 'ui-fonts', title: '界面字体与文字颜色', icon: 'type', html: `<p>设置 → 界面字体与文字颜色。内置 IBM Plex Sans SC，附字体许可证；在线字体库提供 20 款界面字体，确认后下载并使用。内置和已下载字体立即切换，导入的 TTF / OTF / WOFF / WOFF2 界面字体需要完整退出后重新启动才能生效。浅色、深色模式的界面文字颜色分别保存；界面字体与终端字体分别管理。</p>` },
    ...
    { id: 'start', title: '从第一台服务器开始', icon: 'terminal', html: `<p>DengShell 把 SSH 终端、远程文件和服务器状态放在同一工作台。顶部切换会话，左侧查看状态，右侧使用终端，下方管理文件或快捷命令。</p><ol><li>点击顶部文件夹图标，进入<strong>服务器管理</strong>，选择“新建连接”。</li><li>填写名称、服务器地址、端口和用户名，选择所属分组及认证方式。</li><li>按需要选择密钥、保存凭据或配置代理，再点击“保存并连接”。</li><li>连接成功后即可使用终端；服务器提供 SFTP 时可使用文件面板。救援或安装环境缺少 SFTP 时，仍可正常使用 SSH。以后双击已保存的服务器可再次连接。</li></ol><p>未连接时，监控显示“—”，文件列表为空，这是等待连接的状态。手册和引导在未连接时也可以查看。</p>` },
    ...
    { id: 'security-lock', title: '安全锁定', icon: 'lock', html: `<p>设置 → <strong>安全锁定</strong>。密码至少 4 位，可设置密码提醒；也可用验证器 App 扫码绑定 2FA，输入当前 6 位验证码并保存。两种方式都启用时，可任选其一解锁。</p><p>选择仅手动上锁，或无操作多少秒后自动锁定。明暗模式右侧的锁形按钮立即锁定全部窗口，界面深度模糊；点击“解锁”后输入密码或验证码。后台监控、终端输出不会延长计时，启用后重启也需要解锁。修改或关闭设置需验证现有凭据。</p><p>已使用的验证码不能重复使用，请等待下一组。连续输错会暂时限制重试。SSH 连接和已开始的任务继续运行，外部编辑器不在锁定范围内。请妥善保管密码或验证器；完整备份需保留整个数据目录，安全锁定不会随连接配置导入导出。</p>` },
    ...
    { id: 'monitor', title: '系统、进程与磁盘', icon: 'cpu', html: `<p>左侧上方以对应操作系统图标和完整名称突出显示发行版，并显示当前服务器的运行时间、CPU、内存、交换空间和负载；活跃进程常驻显示前五，可按 CPU 或内存切换排名。点击进程区域打开独立的完整进程标签，支持按 PID、名称或状态搜索，以及 CPU、内存、PID、状态和名称排序。每页显示 50、100 或 200 行；每个服务器保留自己的搜索和分页。可暂停自动刷新，切走进程标签后停止该页面请求。完整列表的 RSS 使用轻量内核计数，估算值带“≈”；侧栏只对前五候选精读内存。大量进程或采集较慢时自动降低刷新频率，失败会保留上次数据并提示。</p><p>网络流量每<strong>1秒</strong>独立采样，负载、CPU、内存和磁盘速率每<strong>5秒</strong>采样；网卡元信息和磁盘容量约30秒刷新。每条已连接SSH的网卡流量在后台持续记录，切换标签或最小化也保留最近30秒真实历史；回到标签时立即显示。负载、CPU、内存和磁盘等其它采样仍按可见状态请求。网络使用远端实际时间差计算，SSH或TUN返回延迟不会被当成远端采样间隔。读数明确标注“上行”和“下行”，例如上行4.0 KiB/s、下行819 B/s。默认上行橙色实线、下行蓝色虚线，可在“设置 → 曲线样式”中分别调整颜色、粗细和线型；刻度按最近<strong>60秒</strong>数据自动切换B/s、KiB/s、MiB/s、GiB/s刻度（1024进位），新峰值立即扩大范围，较小范围稳定5秒后缩小。缺测保留断点，不填入假零值。下方磁盘区域汇总已挂载本地磁盘文件系统的已用、可用和总计空间，包括独立的 /home、/data 等分区；同一文件系统的重复挂载只计算一次，排除内存盘、网络挂载、容器叠加层及 SWAP。容量以文件系统报告值为准，不包含未挂载空间，可用空间可能因文件系统保留空间而少于总计减已用。容器或存储池根文件系统无法安全合计时，保留根文件系统容量。读写速度仍显示在下方，悬停容量区域可查看参与汇总的路径。</p><p>IPv4/IPv6面板只显示可确认的公网地址，点击复制，悬停查看来源；可能显示SSH入口或网卡地址。NAT入口不等于服务器出口，无法确认时显示“—”。真实内网地址仍保留在网卡菜单提示中。</p><p>这些数值来自远程服务器的周期采样，不是本机硬件数据。刚连接时需要等待首轮采样；部分权限或系统接口不可用时可能显示“—”。较窄窗口可通过顶部芯片图标打开侧栏。</p>` },
  ...
  if (window.DENGSHELL_ANDROID) {
    // The desktop guide contains window, mouse and filesystem instructions
    // which cannot be followed in the Android container.
    for (const id of ['finalshell-import', 'tray-prompt']) {
      const index = sections.findIndex(section => section.id === id);
      if (index >= 0) sections.splice(index, 1);
    }
    const copy = {
      start: '底部导航依次打开服务器、终端、文件、监控和更多。横屏时导航移到左侧，旁边显示会话与已保存服务器。先在“服务器”中新增连接，填写地址、端口、用户名及密码或私钥，再点“保存并连接”。连接成功后使用终端；服务器支持 SFTP 时还可管理远程文件。',
      groups: '在“服务器”中搜索、创建分组或新增连接。点击连接卡片的操作按钮可编辑和删除；点击连接可发起 SSH。横屏左侧也会显示已保存服务器。触屏操作无需鼠标右键或键盘组合键。',
      keys: 'Android 支持密码和私钥认证，不支持桌面 SSH Agent。私钥可在“更多 → 密钥管理”中从设备文件选择，或粘贴文本，也可以生成 Ed25519 密钥。首次连接时请先核对服务器主机指纹。',
      terminal: '连接后在终端输入，底部输入条可发送命令。快捷键栏提供 Esc、Tab、Ctrl+C、Ctrl+D 和方向键。顶部可切换会话；横屏左侧也可切换。执行前确认当前服务器。',
      files: '“文件”使用 SFTP 浏览远程目录。上传文件按钮会打开 Android 文件选择器，可一次选择多个文件；目前不支持从设备选择整个文件夹上传。选中文件后可下载、重命名或删除，下载时选择 Android 保存位置。传输任务页显示进度和结果。',
      monitor: '“监控”展示远程服务器的 CPU、内存、进程、网卡流量和磁盘状态。数据来自远程采样，连接后可能需要稍等。Android 本机没有 ICMP ping 工具，因此不显示本机到服务器的延迟曲线。',
      network: 'Android 不支持本机 MTR。连接服务器后可从“更多 → 远程网络诊断”指定目标，让远程服务器执行 MTR；远程服务器可能需要先安装 mtr。单个中间路由器不回应，不一定代表业务流量丢包。',
      appearance: 'Android 版会根据竖屏和横屏自动排布，保持适合触控的字号与按钮尺寸。背景和字体可在设置中调整。',
      config: 'Android 配置保存在应用私有目录。安装同一签名的新 APK 通常会保留应用数据；卸载应用或换用不同签名时数据可能被清除。保存重要服务器信息并谨慎卸载。',
      startup: 'Android 版暂不支持应用内自动安装更新。更新时下载并安装新 APK；先确认与已安装版使用相同签名，必要时备份配置。',
      'file-editor': '连接支持 SFTP 的服务器后，可打开远程文本文件进行编辑。保存前请确认目标服务器与路径；其他程序修改同一文件时需处理冲突。Android 不支持桌面外部编辑器。',
    };
    for (const section of sections) if (copy[section.id]) section.html = `<p>${copy[section.id]}</p>`;
  }
  const steps = [
    { section: 'start', title: '先认识你的 SSH 工作台', target: '.terminal-panel', fallback: '#settings-button', location: '顶部会话 · 左侧状态 · 右侧终端 · 下方文件', text: '把常用操作集中在一个窗口。接下来用 12 步走过主要功能；未连接服务器也可以完整查看。', bullets: ['每一步只介绍操作入口，不会替你连接或执行命令。', '随时点击“暂时结束”或按 Esc，之后可从使用手册重新查看。'] },
    ...
    { section: 'quick-commands', title: '把常用操作保存为快捷命令', target: '[data-pane="commands"]', location: '下方面板 → 命令', text: '命令区域右键可新建分组，空组也会保留。为命令设置名称、分组、颜色和内容，每条命令显示为彩色胶囊。', bullets: ['新建或编辑命令时可选择已有分组，分组旁“＋”可创建新组。命令默认不追加回车，勾选回车后才自动执行；带参数时先填写参数。', '发送前确认顶部会话，避免选错服务器。'] },
    ...
    { section: 'monitor', title: '查看系统资源和磁盘空间', target: '.sidebar', fallback: '#toggle-monitor', location: '左侧状态栏；窄窗口用顶部芯片按钮打开', text: 'CPU、内存、负载、运行时间、活动进程、网卡流量和磁盘容量来自当前远程服务器的周期采样。', bullets: ['选择网卡查看流量，常驻显示占用前五进程，点击 CPU / 内存列标题切换排名。', '未连接或尚未采样时显示“—”，连接成功后会刷新。'] },
    ...
    { section: 'fonts', title: '每款字体都有自己的加粗和颜色', target: '#settings-button', location: '左上角设置 → Shell 字体与颜色 / 背景管理器', text: '内置五款 Shell 字体，可从在线字体库选择 30 款字体，也可导入字体和背景。每张字体卡片单独保存加粗与文字颜色，支持预设、RGB 和 HEX。', bullets: ['选择当前字体立即预览；字号按 0.5 px 调整，面板可拖动。', '背景可以自定义并调整强度，文字的 ANSI 颜色仍由远程程序决定。'] },
    { section: 'appearance', title: '调整主题、布局和整体大小', target: '#theme-toggle', location: '左上角主题按钮；设置 → 恢复默认布局 / 界面缩放', text: '以开关灯效果切换浅色或深色主题，拖动分隔条调整终端、侧栏和文件区比例。界面缩放与终端字号可以分别调整。', bullets: ['小窗口会适当限制实际缩放，保证控件仍然可用。', '设置中的“恢复默认布局”恢复布局和终端字号；启动动画也可在设置中调整。'] },
    { section: 'config', title: '保留配置，随时回来查手册', target: '#settings-button', location: '左上角设置 → 连接配置文档 / 使用手册与功能引导', text: '升级时在同一目录替换程序。备份或换电脑时，一起保留加密配置、解锁密钥和 assets / keys 资源目录。', bullets: ['启动检查最多 3 秒；按 SHA-256 比较固定更新包，确认后下载、校验并重启安装。', '本手册逐项说明功能并支持搜索，也可以重新播放这份引导。'] },
  ];
  ...
  function positionTarget() {
    ...
    const step = steps[stepIndex], primary = $h(step.target), target = visible(primary) ? primary : $h(step.fallback || '#settings-button');
  ...
  function openManual(sectionID = '') {
    ...
    if (!dialog.open) { manualReturn = document.activeElement.closest('#settings-menu') ? $h('#settings-button') : document.activeElement; dialog.showModal(); }
  ...
  async function startFirstGuide() {
    if (window.DENGSHELL_ANDROID) return;
