...
function reflectServerSelection() {
  ...
  hint.textContent = window.DENGSHELL_ANDROID ? '点按连接 · 使用右侧 ⋯ 管理' : selected.size ? `已选 ${selected.size} 台 · Enter 打开` : '单击选择 · 双击打开 · Ctrl / ⌘ 多选';
}
...
function renderServerProfile(profile, tree, mode = 'servers', history = null) {
  ...
  button.setAttribute('aria-label', `${deleted ? '已删除' : window.DENGSHELL_ANDROID ? '连接' : '选择'} ${profile.name}`);
  ...
  button.onclick = safe(event => {
    if (window.DENGSHELL_ANDROID && !deleted && !event.ctrlKey && !event.metaKey) {
  ...
  button.ondblclick = safe(event => { if (!window.DENGSHELL_ANDROID && !deleted && !event.ctrlKey && !event.metaKey) { clearServerSelection(); return connect(profile.id); } });
