...
window.DengFileBrowser = (() => {
  ...
  async function readDirectory(state, path, force = false) {
    if (!state?.connected || state.sftpPending || state.sftpAvailable === false) return;
  ...
  function render(state = current()) {
    ...
    if (state.sftpPending) { container.replaceChildren(node('div', 'tree-placeholder', '正在加载服务器目录…')); delete container.dataset.session; return; }
