...
(() => {
 ...
 function showChoice(){
  if(dialog?.open)return;window.DengShellSplash?.finish('interaction');dialog=document.createElement('dialog');dialog.className='manager-dialog tray-choice-dialog';dialog.setAttribute('aria-labelledby','tray-choice-title');dialog.innerHTML='<div class="dialog-heading"><h2 id="tray-choice-title">最小化到哪里？</h2></div><p>可隐藏到系统托盘继续保持 SSH 连接，也可以保留在任务栏。</p><p class="tray-unavailable-note" hidden>当前桌面没有可用托盘，将使用普通最小化。</p><label class="tray-remember"><input type="checkbox" checked><span>记住我的选择，可在设置中修改</span></label><div class="tray-choice-actions"><button type="button" data-choice="cancel">取消</button><button type="button" data-choice="minimize">保留在任务栏</button><button type="button" class="primary-button" data-choice="tray">隐藏到托盘</button></div>';
