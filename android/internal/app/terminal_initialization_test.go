...
// A terminal-first connection owns only a local bootstrap plan before attach.
// Closing it must not stage or remove remote paths; observe through an independent
// disposable SSH session with explicitly initialized SFTP.
func TestPrewarmedTerminalCloseBeforeAttach(t *testing.T) {
	...
	if s.Home == "" || integration.Shell != "" || !integration.inline || integration.command == "" || s.preparedIntegration == nil {
		t.Fatal("incomplete local launch plan")
	}
	if files, _ := s.fileClientSnapshot(); files != nil || !s.SFTPPending || s.fileBridge.Load() != nil {
		t.Fatal("unattached connection opened optional file service")
	}
	if s.prepareTerminalIntegration().Nonce != integration.Nonce {
		t.Fatal("bootstrap initialized twice")
	}
	stopObserver := context.AfterFunc(ctx, observer.forceClose)
	defer stopObserver()
	observer.startFileInitialization()
	if err := observer.waitFileInitialization(ctx); err != nil {
		t.Fatal("observer SFTP initialization", err)
	}
	files, _ := observer.fileClientSnapshot()
	if files == nil {
		t.Fatal("observer SFTP unavailable")
	}
	assertAbsent := func(stage string) {
		t.Helper()
		for _, name := range append(append([]string(nil), integration.files...), integration.directory) {
			if _, err := files.Lstat(name); !os.IsNotExist(err) {
				t.Fatalf("%s connection staged %s: %v", stage, filepath.Base(name), err)
			}
		}
	}
	assertAbsent("unattached")
	a.disconnect(s.ID)
	assertAbsent("closed unattached")
}
