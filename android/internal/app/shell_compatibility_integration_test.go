...
func TestShellCompatibilitySSHMatrix(t *testing.T) {
	...
	for _, shell := range []string{"bash", "zsh", "fish"} {
		t.Run(shell, func(t *testing.T) {
			...
			if integration.Shell != "" || integration.Nonce == "" || !integration.inline || integration.command == "" {
				t.Fatalf("missing local inline launch plan: %+v", integration)
			}
			...
			wait(prompt, 0)
			// This fixture starts the terminal directly, bypassing the relay that
			// normally confirms the nonce and initializes optional file channels.
			...
			if err := s.waitFileInitialization(ctx); err != nil {
				t.Fatal("matrix SFTP initialization", err)
			}
			files, home := s.fileClientSnapshot()
			if files == nil {
				t.Fatal("matrix SFTP unavailable")
			}
			if _, err := files.Stat(integration.directory); !os.IsNotExist(err) {
			...
			directory := path.Join(home, "cwd space%#?中文\nline")
			if err := files.Mkdir(directory); err != nil {
				t.Fatal(err)
			}
			defer files.RemoveDirectory(directory)
