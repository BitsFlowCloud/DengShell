...
func TestOptionalSFTPKeepsSSHUsable(t *testing.T) {
	for _, mode := range []string{"reject", "request-stall", "close", "handshake-stall", "home-stall", "available"} {
		t.Run(mode, func(t *testing.T) {
			...
			if !s.SFTPPending || s.SFTPAvailable || s.files != nil {
				t.Fatal("file initialization must wait for the terminal")
			}
			if s.Home == "" {
			...
			waitCtx, stop := context.WithTimeout(context.Background(), 7*time.Second)
			defer stop()
			if err := s.waitFileInitialization(waitCtx); err != nil {
				t.Fatal(err)
			}
			files, _ := s.fileClientSnapshot()
			if (files != nil) != (mode == "available") {
				t.Fatal("incorrect file readiness")
			}
			// Initial connection metadata is immutable even after the worker ends.
			if !s.SFTPPending || s.SFTPAvailable {
				t.Fatal("connection response fields changed")
			}
			request := httptest.NewRequest("GET", hs.URL+"/api/sessions/"+s.ID+"/file-status", nil)
			request.Header.Set("X-CloudShell-Token", a.Token())
			result := httptest.NewRecorder()
			a.Handler(fstest.MapFS{}).ServeHTTP(result, request)
			if result.Code != 200 || !strings.Contains(result.Body.String(), `"sftpPending":false`) || strings.Contains(result.Body.String(), `"sftpAvailable":true`) != (mode == "available") {
				t.Fatalf("bad status: %d %s", result.Code, result.Body.String())
			}
			if files == nil {
	...
	for _, mode := range []string{"available", "reject"} {
		t.Run(mode, func(t *testing.T) {
			...
			if mode == "available" {
				waitCtx, stop := context.WithTimeout(context.Background(), 7*time.Second)
				defer stop()
				if err := second.waitFileInitialization(waitCtx); err != nil {
					t.Fatal(err)
				}
...
func TestBackgroundFileInitializationCancelledWithSession(t *testing.T) {
	a, host, port := optionalSFTPFixture(t, "home-stall")
	p, err := a.store.Save(Profile{Name: "cancel background files", Host: host, Port: port, User: "fixture", Auth: "password", Secret: "fixture"}, false)
	if err != nil {
		t.Fatal(err)
	}
	s, err := a.Connect(context.Background(), p.ID, "")
	...
	awaitNetworkCondition(t, time.Second, func() bool { return s.fileBridge.Load() != nil })
	s.forceClose()
	select {
	case <-s.fileDone:
	case <-time.After(time.Second):
		t.Fatal("closed session retained background initialization")
	}
	files, _ := s.fileClientSnapshot()
	if files != nil {
		t.Fatal("closed session published a late file client")
	}
	if err := s.waitFileInitialization(context.Background()); err != context.Canceled {
		t.Fatal("closed session file wait did not stop", err)
	}
}
