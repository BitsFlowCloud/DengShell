...
func (s *Session) removePromptStyleBeforeClose() {
	...
	s.mu.Unlock()
	if s.preparedIntegration != nil && s.preparedIntegration.directory != "" && !s.preparedIntegration.inline && !terminalReady {
	...
	go func() { defer close(done); _ = files.Remove(stylePath); _ = files.Remove(stylePath + ".new") }()
