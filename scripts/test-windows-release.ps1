...
function Seed-QA([string]$Config){New-Item -ItemType Directory -Force $Config | Out-Null;@{schemaVersion=2;servers=@();groups=@('QA');appearance=@{minimizeAction='tray';onboardingCompleted=$true;startupAnimation=$false}} | ConvertTo-Json -Depth 8 | Set-Content -Encoding utf8 (Join-Path $Config 'config.json');Set-Content -Encoding utf8 (Join-Path $Config 'keep-user-data.txt') 'preserve-local-fixture'}
